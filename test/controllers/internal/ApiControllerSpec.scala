...
class ApiControllerSpec extends UnitTestSpec with MockMessages with MockAppConfig with AuthHelpers {
  ...
  "journeyInitialisation" should {
    ...
    val requestWithSessionId: FakeRequest[JsValue] =
      FakeRequest().withSessionId("test-session-id").withBody(validRequestedJson)
  ...
  "fetchResults" must {
    ...
    val journeyData = JourneyData(Identifiers(journeyId, sessionId), "redirectUrl", JourneySetup(), Instant.now())
