...
class FeatureSwitchProviderConfig @Inject() (config: AppConfig) {
  ...
  lazy val frontendFeatureSwitchProvider: FeatureSwitchProvider = FeatureSwitchProvider(
    id = "industry-classification-lookup-frontend",
    appName = "ICL Frontend",
    url = frontendFeatureSwitchUrl
  )

  lazy val featureSwitchProviders: Seq[FeatureSwitchProvider] = Seq(frontendFeatureSwitchProvider)
}
