...
@Singleton
class ChooseActivityController @Inject() (
  ...
  def show(journeyId: String, doSearch: Option[Boolean] = None): Action[AnyContent] = Action.async { implicit request =>
    userAuthorised() {
          ...
          withSearchResults(journeyData.identifiers) { searchResults =>
            Future.successful(
              Ok(
                view(
                  journeyId,
                  SicSearchForm.form.fill(SicSearch(searchResults.query)),
                  chooseActivityForm,
                  Some(searchResults)
                )
              )
  ...
  private[controllers] def selectActivities(journeyId: String, journeyData: JourneyData)(implicit
    request: Request[_]
  ): Future[Result] = {
    withSearchResults(journeyData.identifiers) { searchResults =>
      ChooseMultipleActivitiesForm.form(Some(searchResults)).bindFromRequest().fold(
        errors =>
          Future.successful(
            BadRequest(
              view(journeyId, SicSearchForm.form.fill(SicSearch(searchResults.query)), errors, Some(searchResults))
            )
          ),
        codes =>
  ...
  private def filterResultsBySector(journeyId: String, sectorCode: Option[String])(implicit
    hc: HeaderCarrier,
  ...
  private[controllers] def withSearchResults(
    identifiers: Identifiers
  )(f: => SearchResults => Future[Result])(implicit request: Request[_]): Future[Result] =
